...
\begin{enumerate}
    \item Install the four Holybro 2216 880KV brushless motors on the arm motor mounts following the CW/CCW positions specified for the S500 V2 kit (see the motor diagram in the User Manual, Appendix~D).
...
The Arducam IMX219 connects to the Jetson CSI0 port via the 22-pin to 15-pin ribbon cable. Install the Arducam driver~\cite{arducam_imx219}:
...
\begin{enumerate}
    ...
    \item Verify each motor spins in the correct direction (see motor diagram in the User Manual, Appendix~D): M1 Front-Right CCW, M2 Back-Left CW, M3 Front-Left CW, M4 Back-Right CCW.
