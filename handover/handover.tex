...
Camera & Arducam IMX477 & 12\,MP, 100$^\circ$ FOV, MIPI CSI \\
...
CSI ribbon cable & 22-pin, 0.5\,mm pitch & Arducam IMX477 to Jetson CSI0 (camera0) \\
...
Camera mount & 1 & Attaches to front of frame; holds Arducam IMX477 facing forward \\
...
\begin{enumerate}
    ...
    \item Install the Arducam IMX477 camera in the mount with the lens facing forward.
    \item Attach the mount to the front of the frame.
    \item Route the 22-pin 0.5\,mm CSI ribbon cable from the camera toward the Jetson CSI0 connector (camera0). Take care not to kink or crease the ribbon cable; leave a gentle loop for vibration relief.
\end{enumerate}
...
Camera CSI & Arducam IMX477 & Jetson Orin Nano CSI0 (camera0) & 22-pin, 0.5\,mm ribbon \\
\bottomrule
\end{longtable}
...
The Jetson connects to the Pixhawk via a custom UART cable: Jetson GPIO TX and RX pins (40-pin header) to Pixhawk TELEM2 (JST-GH 6-pin). Wire TX on the Jetson to RX on the Pixhawk, RX on the Jetson to TX on the Pixhawk, and GND to GND. On the Jetson Orin Nano, the UART appears as \texttt{/dev/ttyTHS1}. The PX4 TELEM2 baud rate must be set to 921600 via the \texttt{SER\_TEL2\_BAUD} parameter (see Section~\ref{sec:params}). If the MAVROS connection fails on startup, verify the device node exists with \texttt{ls /dev/ttyTHS*} and that the user has serial port permissions (\texttt{sudo usermod -aG dialout \$USER}, then log out and back in).
...
\subsection{Camera Setup (Arducam IMX477)}

The Arducam IMX477 connects to the Jetson CSI0 port via the 22-pin ribbon cable. Install the Arducam driver:

\begin{lstlisting}[language=bash]
# Follow the official Arducam Jetson installation guide for the IMX477
...
\begin{enumerate}
    \item Run the camera test scripts from \texttt{tests/} to verify autofocus and image capture. These can be run on the ground with the drone stationary.
...
\bibitem{arducam_imx477}
    Arducam,
    \emph{IMX477 Camera for NVIDIA Jetson -- Quick Start Guide},
